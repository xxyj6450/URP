...
alter PROC Interrupt_deliverydoc
    (
      @doccode VARCHAR(50) ,
      @source VARCHAR(50) ,
      @user VARCHAR(50) ,
...
AS 
    ...
    SET XACT_ABORT ON  
    BEGIN  
        DECLARE @gooddoccode VARCHAR(50) ,
            @sdorgid VARCHAR(50) ,
            @Source1 VARCHAR(50) ,
        ...
        RETURN
        SELECT  @gooddoccode = refcode ,
                @sdorgid = sdorgid
        FROM    fh_deliverydoc
        WHERE   doccode = @doccode  
        IF LEFT(@refcode, 2) = 'DD' 
            SET @formid = '6090'  
        IF LEFT(@refcode, 2) <> 'DD' 
            SET @formid = '6091'  
 ...
 --检查是否已送货  
        ...
        ELSE 
            BEGIN  
                DELETE  imatdoc_h
                WHERE   ISNULL(usertxt3, '') = @doccode  
  --处理原单，作标识  
                UPDATE  fh_deliverydoc
                SET     memo = '中止分货单' + @user
 ...
 --exec sp_caceldeliverydoc @doccode,@Source  
        ...
        from ' + @Source + '.' + @Source1
            + '.dbo.ord_shopbestgoodsdtl l,fh_deliverydtl m,fh_deliverydoc n                   
 where m.doccode=''' + @doccode
            + ''' and m.doccode=n.doccode and l.matcode=m.matcode and l.doccode=n.refcode'    
 --PRINT '1:'+    @sql                          
        EXEC(@sql)                
                               
 ---更新明细金额                                
        SET @sql = 'update ' + @Source + '.' + @Source1
            + '.dbo.ord_shopbestgoodsdtl set totalmoney=isnull(ask_digit,0)*isnull(salesprice,0) 
           where doccode in(select refcode from fh_deliverydoc where doccode='''
            + @doccode + ''')'              
 --PRINT '2:'+    @sql
        EXEC(@sql)
 ---更新表头汇总数据
        ...
        from ' + @Source + '.' + @Source1 + '.'
            + 'dbo.ord_shopbestgoodsdoc a,                                
          (select doccode, sum(canceldigit) digit,sum(totalmoney) totalmoney from '
            + @Source + '.' + @Source1
            + '.dbo.ord_shopbestgoodsdtl  
			where  doccode in(select refcode from fh_deliverydoc where doccode='''
            + @doccode + ''')         
 ...
 --PRINT '3:'+    @sql         
        ...
        SET @sql = 'update ' + @Source + '.' + @Source1 + '.'
            + 'dbo.ord_shopbestgoodsdoc   set phflag=''未处理'' where doccode in(select refcode from fh_deliverydoc where doccode='''
            + @doccode + ''')'                          
        EXEC(@sql)               
 --PRINT '4:'+    @sql             
 ...
 --exec(@sql)      
 --执行作废订单  
        SELECT  @sql = @source + '.' + @Source1 + '.dbo.sp_updatetpoff '
                + @formid + ',''' + @refcode + ''',''' + @sdorgid + ''','''
                + @user + ''','''''  
 --PRINT '5:'+    @sql     
        EXEC (@sql)  
