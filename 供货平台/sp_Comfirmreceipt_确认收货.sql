...
示例:
begin tran
exec sp_ComfirmReceipt 'JDC2012113000006',4950,'SYSTEM','2.1.769.09.29','102','','1243010B-4C4E-40FE-AAC5-B3AD367AA74D','1',''
 ...
 set xact_abort on
 begin tran
exec sp_ComfirmReceipt 'JDC2012113000000',4950,'SYSTEM','2.1.769.09.29','111.769','','E36D5919-925A-4C78-95FB-B9E0A2007A6F','1',''

begin tran
exec sp_ComfirmReceipt @Doccode='JDC2012113000020',@Formid=4950,@Usercode='7690929',@stcode='2.1.769.09.29',@fhstcode='111.769',@remark='',@InstanceID='E36D5919-925A-4C78-95FB-B9E0A2007A6F',@optionID='1',@TerminalID=''
...
as
	BEGIN
		...
		@DatabaseName VARCHAR(50),@ServerName VARCHAR(50)
			...
			BEGIN
				...
				IF @@ROWCOUNT>0
					BEGIN
						...
						' select i2.seriescode,i2.rowid,i2.matcode,i2.matname,i2.docitem,convert(varchar(20),'''') as state '+dbo.crlf()+
		...
		begin try
			...
			select @tips=@tips+convert(varchar(5),b.docitem)+'.'+b.seriescode+'['+b.matname+'],库存状态为[' +isnull(b.state,'')+']'+dbo.crlf()
			from   #iSeries b
			where isnull(b.state,'已售') not in('已售','送货')
			if @@ROWCOUNT>0
				BEGIN
					raiserror(@tips,16,1)
					return
