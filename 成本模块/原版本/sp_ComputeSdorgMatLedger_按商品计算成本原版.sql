 ...
 */  
 ...
 --将参数中的NULL置空
 select @digit=isnull(@digit,0),@totalmoney=isnull(@totalmoney,0),@ratemoney=isnull(@ratemoney,0)
 BEGIN
  ...
  */
  ...
  IF @mode=1 
 	BEGIN
 		...
 		SET stock=isnull(stock,0)-@digit,StockValue =isnull(stockvalue,0)-isnull(map,0)*@digit,ratevalue = isnull(ratevalue,0)-isnull(ratemap,0)*@digit,
  ...
  IF @mode=2
 	BEGIN
 		...
 		*/
 		UPDATE iMatsdorgLedger SET stock=isnull(stock,0)-@digit,StockValue =isnull(stockvalue,0)-isnull(map,0)*@digit,ratevalue = isnull(ratevalue,0)-isnull(ratemap,0)*@digit,
 		ModifyDate=getdate(),ModifyDoccode=@doccode
 		output @sdorgid,inserted.matcode,@RowID,deleted.stock,deleted.stockvalue,deleted.ratevalue,@Digit,@TotalMoney,@Ratemoney,inserted.stock,inserted.stockvalue,inserted.ratevalue,@Mode,@Type into @table
 		WHERE  sdorgid=@sdorgid AND matcode=@matcode
		if @@Rowcount=0
