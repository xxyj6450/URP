...
BEGIN
					...
					;with cte as(
						...
						update a
							set a.totalmoney=isnull(
													abs(
														coalesce(
																	nullif(1.0000*i.outledgeramount/nullif(i.outledgerdigit,0),0),
																	nullif(1.0000*i.inledgeramount/nullif(i.inledgerdigit,0),0)
																)
														)
													,0),
						ratemoney=isnull(
											nullif(
													isnull(abs(coalesce(nullif(1.0000*i.outrateamount/nullif(i.outledgerdigit,0),0),nullif(1.0000*i.inrateamount/nullif(i.inledgerdigit,0),0))),0)
												,0)
	...
	BEGIN
		...
		--再计算内部采购入库成本
		insert into #table(matcode,Rowid,digit,totalmoney,ratemoney,Mode,ComputeType)
		select a.MatCode,a.rowid,b.digit,b.netmoney,1.0000*b.digit*b.netprice*((100+img.addpresent)/100),3,''
	...
	BEGIN
		...
		insert into #table(matcode,Rowid,digit,totalmoney,ratemoney,Mode,ComputeType)
		select b.MatCode,b.rowid,b.digit,a.netmoney,1.0000*b.digit*a.netprice/((100+img.addpresent)/100),4,''
		from imatdoc_d a with(nolock) inner join sPickorderitem  b with(nolock) 
			on a.MatCode=b.MatCode
